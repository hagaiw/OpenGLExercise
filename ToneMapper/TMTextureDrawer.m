...
- (void)drawWithTextureProgram:(TMTextureProgram *)program
  ...
  [program use];
  [frameBuffer bind];
  [texture bindAndlinkToHandle:program.textureUniform withTextureType:TMTextureTypeDefault];
  ...
  [program useProgramWithUniforms:[self uniformsWithMergedProjectionsFromUniforms:uniforms]];
...
- (NSArray *)uniformsWithMergedProjectionsFromUniforms:(NSArray *)uniforms {
  NSMutableArray *newUniforms = [[NSMutableArray alloc] init];
  ...
  for (id<TMUniform> uniform in uniforms) {
    if ([uniform.name isEqualToString:kProjectionUniform]) {
      ...
      [newUniforms addObject:uniform];
    }
  }
  [newUniforms addObject:projection];
  return newUniforms;
}
